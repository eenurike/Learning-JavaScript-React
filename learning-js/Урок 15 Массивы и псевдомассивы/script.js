...
// Часто при работе с массивами мы хотим его просто редактировать, либо хотим добавить, либо удалить элемеент с массива и для этого у нас есть методы как для работы с концом массива так и с его началом
...
// Здесь нужно напомнить, когда разбирали обьекты, говорили что мы не могли их перебрать при помощи for of, дело в том что такая конструкция работает с массиво подобнвми сущностями: массив, строка, псевдомассив и такие сущности как map и set, так же мы можем какие-то определенные элементы со страницы

const arr3 = [1, 2, 3, 4, 5, 6, 7];

console.log(arr3.length); // 7
// Но ведь помним что массив начинается с 0, так что должно получится 6 элементов, что не логично. Дело в том что свойство length состоит из последнего индекса котрый есть у нас в массиве +1. Но и давайте проговорим некоторые нарушение из правил которые могут возникнуть в коде, мы всегда помним что массивы служат для того чтобы все эдементы хранить строго по порядку, но иногда эти правила случайно могут нарушаться
...
console.log(arr3);  // [ 1, 2, 3, 4, 5, 6, 7, <92 empty items>, 0 ] мы получаем 92 пустых ячейки, это одно из главных нарушений смысла нащего массива, то что у нас элементы лежат не попорядку

// Метод массивов который используется больще всех в коде, метод forEach, этот метод позволляет нам гибко перебрать все элементы котрые находятся в массиве, по факту он служит точно так же как и эти два способа выше, но с некотрыми отличиями, мы его можем более подробно настроить
...
const str = prompt('');
// От пользователя будем получать какую-то строку, и дальше на основании этой строки, хотиим сформировать список товаров
...
// Поучится только с правильным указанным разделителем, в нашем случае запятая пробел, а если разделитель будет отличаться, он просто запишет ее как в строку как тип данных string
...
const result = str4.join('; '); // Разделитель точка с запятой и пробелом
...
// Изначально метод sort сортирует все как строки, каждое число которое начинается с 1, встала впереди, причем сравнение шло посимвольно. 1, 10, 13 у этих трех чисел первая 1, так что он равны, потом 0 и 3, 0 будет меньще 3 поэтому 10 встала на второе место, потом идет у нас 13, массив с числами отсортировался как строки, но этого мы можем избежать. Метод sort может принимать в себя callback функцию, мы можем сказать как именно отсортировать этот массив, сейчас мы наппишем шаблон котрый очень часто будет встречаться в таких сортировачных вещах

function compareNum(a, b) {
  return a - b;
}

console.log(sortNumber.sort(compareNum)); // [1, 2, 3, 4, 10, 13, 23] вот теперь числа отсортировались правильно. И возникает вопрос, а как вооюще работает эта функация. Дело в том что метод sort внутри себя использет один из алгоритмов, который называется алгоритм быстрой сортировки, это не совсем простая вещь и на практике она не особо нужна, просто запомни эту фукнцию и используй, но для того чтобы углубится в знание алгоритмов, стоит ознакомится с данным алгоритмом по ссылке в уроке
