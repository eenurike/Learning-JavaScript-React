...
let und;
console.log(und); // В переменной und ничего не лежит она пустая, ну и результат undefined. И это значение мы можем уже получать в консоли и на сайтах, так как переменная существует, но она просто пустая 
...
console.log(arrayObj[0]); //  a, такой способ позволяет избегать ошибок. Поэтому если вдруг будут не предвиденные обстоятельства с обьектами, то всегда надежнее использовать квадратные скобки, они работают нормально и с цифрами которые бугут попасть, или если попадется кирилица или вообще какойто другой язык, и в таких ситуациях будет надежным использовать квадратные скобки
