...
// Я сыт!, Наше условие выполнилось так как оба условия выполнились. То есть при помощи логического оператора И, мы проверяем два сразу условия. Как итог заметь (hamburger && fries) тут мы получили полностью правдивый результат, то есть это условие нам вернуло правду, и поэтому выполнилось действие Я сыт!
...
// в консоле ничего не вывелось, потому что условие не выполнилось, а у нас нет блока else и код просто идет дальше. Получается так что вы сможете сьесть только одно блюдо, но вместе с картошкой не сможете этого сделать, и условие которое мы проверяем нам вернуло false

console.log((hamburger && fries)); // false

// Это такое базовое применение, когда мы четко видим что оба аргумента которые здесь используются они как бы сразу в логическом типе true/false, но так бывает далеко не всегда 
...
// Пример из реальной практики, например хочешь посмотреть какой-то фильм на пиратском сайте, и там обычно нужно посмотреть несколько реклам, то есть мы можем сделать условие видео не разблокируется до тех пор, пока пользователь не досмотрел рекламу 1 и рекламу 2, и опять будет использоваться этот оператор И в условие
...
cola = 0;
let nuggets = 2;
// Нам необходимо чтоб были ИЛИ 3 гамбургера И две колы ИЛИ 3 картошки И хотябы одна порция нагедсов

if (hamburger === 3 && cola === 2 || fries === 3 && nuggets) {
  // Смотри в таблицу преоритетов, сперва выполняется оператор сравнения, затем логический И(&&), потом логический ИЛИ(||)
  console.log('Все довольны');
} else {
  console.log('Мы уходим щщс');
}
// Все довольны, hamburger === 3 && cola === 2 FALSE, fries === 3 && nuggets TRUE, и логический ИЛИ(||) запинается о правду, так что и получаем TRUE
