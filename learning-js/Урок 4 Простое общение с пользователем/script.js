...
alert('Hello');
// И видим что у нас выскакивает небольшое модальное окошко в котором написан тот текст который мы в него передали, и мы должны понимать что это модальное окно оно встроенно в браузер и к сожалению мы его никак не можем застилизовать и для каждого браузера оно будет выглядеть по разному, то же самое и для двух команд которые мы разберем дальше. Вообще команда alert используется чтоб предупредить пользователя о чемто важном, но будем говорить честно описанные в этом уроке возможности используются довольно таки редко в таких больших комплексных проектах. Для этого у нас конечно будет написан интерфейс который будет написан на HTML, но все равно мы должны знать эти команды, это самая база JS

// const result = confirm('Are you here?');
// Задаем пользователю вопрос, на который получаем ответ в виде ДА/НЕТ, тоесть тип данных BOOLEAN
// console.log(result); 
...
// И такие модальные окна, они существуют только внутри браузера, то есть если попытатсья вывести такие конструкции через Run Code получим ошибку, потомучто консоль в VSCode совершенно не знать что такое prompt, alert, confirm, document, window 
